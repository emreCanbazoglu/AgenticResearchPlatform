...
**Claude Code is the sole director of this project.** It owns:
...
**OpenAI Codex agents handle implementation.** Claude Code spawns them via CLI to execute well-defined tasks, then reviews their output before merging.

### Workflow

1. Claude Code breaks work into a scoped task with clear inputs, outputs, and acceptance criteria.
2. Claude Code spawns a Codex agent via CLI: `codex "<task description>"` (or with `--approval-mode auto-edit` for autonomous runs).
3. Codex implements the task on a branch.
4. Claude Code reviews the diff, runs tests, and either approves or sends back with revision notes.
5. Claude Code merges approved work and updates project state.

### Division of Responsibilities

| Responsibility | Owner |
|---|---|
| Spec writing | Claude Code |
| Task decomposition | Claude Code |
| Architecture decisions | Claude Code |
| Implementation | Codex agent |
| Code review | Claude Code |
| Test validation | Claude Code |
| Merge approval | Claude Code |
